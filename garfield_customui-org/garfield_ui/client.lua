...
AddEventHandler('esx:playerLoaded', function(xPlayer) 
	...
	for k, v in pairs(accounts) do
		...
		elseif account.name == "black_money" then
			SendNUIMessage({action = "setValue", key = "dirtymoney", value ="Argent sale $" .. account.money})
		end
	end
	SendNUIMessage({action = "setValue", key = "job", value = data.job.label .. " - " .. data.job.grade_label})
	SendNUIMessage({action = "setValue", key = "job2", value = data.org.label .. " - " .. data.org.grade_label})
...
RegisterNetEvent('esx:setJob')
AddEventHandler('esx:setJob', function(job)
	SendNUIMessage({action = "setValue", key = "job2", value = userData.org.label .. " - " .. org.grade_label})
end)

RegisterNetEvent('es:activateMoney')
AddEventHandler('es:activateMoney', function(e)
	SendNUIMessage({action = "setValue", key = "money", value = "Liquide $" .. e})
end)
